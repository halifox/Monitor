...
class CapabilitiesParser {
  ...
  ParsedCapabilities parse(String raw) {
    ...
    final Set<int> commands = _parseHexList(_extractSection(normalized, 'cmds'));
    final _VcpSection vcp = _parseVcpSection(_extractSection(normalized, 'vcp'));
  ...
  Set<int> _parseHexList(String? raw) {
    ...
    final Iterable<RegExpMatch> matches = RegExp(r'[0-9A-Fa-f]{2}').allMatches(raw);
    return matches
  ...
  _VcpSection _parseVcpSection(String? raw) {
    ...
    while (index < raw.length) {
      ...
      final Match? codeMatch = RegExp(r'^[0-9A-Fa-f]{2}').matchAsPrefix(raw.substring(index));
      if (codeMatch == null) {
        index++;
        continue;
      }
